...
import (
	...
	vv1beta1 "volcano.sh/volcano/pkg/apis/scheduling/v1beta1"
...
func (v *VolcanoBatchScheduler) ShouldSchedule(app *v1beta1.DataxApplication) bool {
	//NOTE: There is no additional requirement for volcano scheduler
	return true
}

func (v *VolcanoBatchScheduler) DoBatchSchedulingOnSubmission(app *v1beta1.DataxApplication) error {
...
func (v *VolcanoBatchScheduler) syncPodGroupInClientMode(app *v1beta1.DataxApplication) error {
	// We only care about the executor pods in client mode
	if _, ok := app.Spec.Executor.Annotations[vv1beta1.KubeGroupNameAnnotationKey]; !ok {
		...
		if err := v.syncPodGroup(app, 1, totalResource); err == nil {
			app.Spec.Executor.Annotations[vv1beta1.KubeGroupNameAnnotationKey] = v.getAppPodGroupName(app)
		} else {
			return err
		}
	}
...
func (v *VolcanoBatchScheduler) syncPodGroupInClusterMode(app *v1beta1.DataxApplication) error {
	...
	if _, ok := app.Spec.Driver.Annotations[vv1beta1.KubeGroupNameAnnotationKey]; !ok {
		...
		if err := v.syncPodGroup(app, 1, totalResource); err == nil {
			app.Spec.Executor.Annotations[vv1beta1.KubeGroupNameAnnotationKey] = v.getAppPodGroupName(app)
			app.Spec.Driver.Annotations[vv1beta1.KubeGroupNameAnnotationKey] = v.getAppPodGroupName(app)
		} else {
			return err
		}
	}
...
func (v *VolcanoBatchScheduler) getAppPodGroupName(app *v1beta1.DataxApplication) string {
	return fmt.Sprintf("datax-%s-pg", app.Name)
}

func (v *VolcanoBatchScheduler) GetAnnotations(app *v1beta1.DataxApplication) map[string]string {
	annotations := make(map[string]string)
	annotations["scheduling.k8s.io/group-name"] = v.getAppPodGroupName(app)
	return annotations
}

func (v *VolcanoBatchScheduler) syncPodGroup(app *v1beta1.DataxApplication, size int32, minResource corev1.ResourceList) error {
	var err error
	...
	if pg, err := v.volcanoClient.SchedulingV1beta1().PodGroups(app.Namespace).Get(context.TODO(), podGroupName, metav1.GetOptions{}); err != nil {
		...
		podGroup := vv1beta1.PodGroup{
			...
			Spec: vv1beta1.PodGroupSpec{
				MinMember:    size,
				MinResources: &minResource,
			},
			Status: vv1beta1.PodGroupStatus{
				Phase: vv1beta1.PodGroupPending,
			},
		}
...
func (v *VolcanoBatchScheduler) CleanupOnCompletion(app *v1beta1.DataxApplication) error {
	podGroupName := v.getAppPodGroupName(app)
...
func getExecutorRequestResource(app *v1beta1.DataxApplication) corev1.ResourceList {
	...
	for i := int32(0); i < *app.Spec.Channel; i++ {
		resourceList = append(resourceList, minResource)
	}
	return sumResourceList(resourceList)
}

func getDriverRequestResource(app *v1beta1.DataxApplication) corev1.ResourceList {
	minResource := corev1.ResourceList{}
